...
function realFor(doc) {
  ...
  return `${loc}${db}DISCIPLINE (non-negotiable): env PROXY_ESTATE_CACHE=/tmp/proxy_estates (REUSE cached real clones — never re-clone: flask@36e4a824, gorilla/mux@v1.8.1, small real Django/Go/TS repos). Use \`.venv/bin/pytest\` or \`.venv/bin/python -m pytest\` for EVERY test/python command — NEVER \`uv run\` or \`uv sync\`: they auto-sync the uv workspace and PRUNE its members (db/contracts/agentkit/...), which breaks every import mid-build and KILLS the run (this already happened once). Every test drives the REAL product entrypoint (run_full_pipeline -> the real tool) on real data — NEVER an injected double (a capability that only works when a test injects it is NOT done). Deterministic-first: scripts decide counts/latency/byte-equality; scoped context (spec section + relevant files, not the whole tree). NEVER run done-check.sh or any >2-min command synchronously — run_in_background + poll the output file ~60s. Guard blocks Edit under tests/acceptance/fixtures/goldens/criteria/product/ — apply those (founder-delegated) via a transparent python/bash script; then decompose + coverage.py must close. Commit each green increment (Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>); do not push.`
}
  ...
  for (let round = 1; round <= MAX_ROUNDS && (!budget.total || budget.remaining() > 150_000); round++) {
    ...
    if (gaps.length === 0) {
      const gate = await agent(
        `Run the forge DONE gate for doc ${doc} and report VERBATIM. ${R} Launch \`bash forge/gates/done-check.sh --spec ${doc}\` (it already uses the venv python directly — never uv run) with run_in_background; poll its output file every ~60s until the process exits; report exitZero and table (the FULL 5-conjunct output, verbatim). Do NOT judge; just run and report.${clock(BUDGETS.gate)}`,
