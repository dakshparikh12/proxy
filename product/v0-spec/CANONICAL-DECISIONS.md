...
CREATE TABLE sessions (id uuid PK, user_id uuid REFERENCES users, tenant_id uuid REFERENCES tenants, created_at timestamptz);  -- DEVIATION (2026-07, founder-approved): relational typed-FK session store. An HMAC-signed cookie carries sessions.id; resolve_session(cookies) → {user_id, tenant_id} via a typed-FK read. Supersedes the original connect-pg-simple (sid text/data jsonb/expire) shape — no consumer needed sid/data/expire, and typed FKs are safer + isolation-provable. `sessions` is NON_SCOPED per AC-TEN-001 (keyed by its own id).
```
