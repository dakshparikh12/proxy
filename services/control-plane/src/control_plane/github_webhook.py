"""The LIVE GitHub push-webhook ingress — the pre-meeting map's freshness caller (§3.6).
...
installation account (Law 3 / the server-side tenant check: a client field can never
authorize an entity). A verified push drives ``_maybe_refresh_map`` — a delta-pull +
pre-meeting map re-build + re-store + re-verify for that repo (honest no-op when the
map-build model seam is unfunded, D-032 — never a fabricated map).

The route never throws (the §4.6 never-throw boundary): a bad signature is a fixed 401,
a non-JSON body from an authenticated caller is its own 400, a non-push event is a benign
202 (accepted, nothing to reindex), and any refresh fault is absorbed so a push delivery
can never 500 the ingress.
"""
...
def install_github_webhook_route(app: "FastAPI") -> None:
    ...
    that triggers NO rebuild (AC-M7-001). A verified push drives ``_maybe_refresh_map`` — a
    delta-pull + pre-meeting map re-build + re-store + re-verify for the SIGNED payload's
    repo, keyed on the AUTHENTICATED installation account (never a request field). The
    tenant is NEVER read from the request.

    Never throws (§4.6): a bad signature is a fixed 401; a non-JSON body is a 400; a
    non-push event is a benign 202 (nothing to reindex); any refresh fault is absorbed so a
    push can never 500 the ingress.
    """
    import json
    ...
    async def github_webhook(request: Request) -> Any:
        ...
        # PRE-MEETING MAP REFRESH: the verified push drives a delta-pull + map re-build +
        # re-store + re-verify for the SIGNED payload's repo (PM-REFRESH-01), keyed on the
        # AUTHENTICATED installation account. Guarded so a map-refresh fault never 500s the
        # ingress. The map-build model seam is credit-blocked (D-032), so this no-ops
        # honestly unless a funded provider + map store are wired onto ``app.state`` (never
        # a fabricated map).
