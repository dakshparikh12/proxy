...
# NOTE: a session-start TRUNCATE fixture for ``meeting_cost_telemetry`` used to live here to
# keep the AC-OBS-003 fixed-id accumulation probe clean across reused-Postgres runs. Migration
# 0011 dropped that table (the workroom pivot left it writer/reader-less), so the fixture — and
# the accumulation hazard it guarded — no longer has anything to reset.
