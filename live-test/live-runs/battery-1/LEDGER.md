...
| F4 | **Follow-up window STILL not firing live** (3rd report) — works in tests w/ the live result shape; live it doesn't. Next diagnosis: instrument the window state (log open/until/checked) on the live path; suspect the wall-clock `ts` passed to _in_follow_up_window vs meeting-relative ts of incoming lines, or the horizon read returning 0 across the process boundary | founder tried post-fix, no wake | running tab |
| F5 | **Audio chop residual** — "works most times, still choppy sometimes" post-backpressure. Next: add live drop/underrun counters (server frames-sent vs page frames-played), check Recall-side capture, consider prebuffer bump | founder ear, post-c249203 | running tab |
| F6 | test-provision (ready-before-join) dies if the HTTP client disconnects mid-assembly (uvicorn cancels the handler; sandbox orphaned, claim stuck) | 42707120 aborted at the 2-min curl timeout | dev-route hardening: shield the assembly from client disconnect |
