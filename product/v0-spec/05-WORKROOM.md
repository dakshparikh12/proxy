...
**`propose_change` is MULTI-FILE (CANONICAL §12.9):** one call stages a whole code-change draft. Signature: `propose_change(kind, summary, files:[{path, old_sha?, new_content}] | unified_diff)` → **one GCS bundle**. The *original* for each file's diff comes from the agent (`old_sha`) or, absent that, from the **pinned clone** at `meeting.pinned_sha`. Persisting at creation is load-bearing (CANONICAL §4): the sandbox's in-memory state dies at teardown, so a human accepting *after* the call needs durable storage. On call it writes the **full bundle (all files / the unified diff) to GCS (Object-Versioned)** + a **`staged_drafts` Postgres row** (schema in CANONICAL §4; `meeting_id` is a **UUID**, CANONICAL §11.2), then returns the `draft_id`. **The ROW is written `status='proposed'`; the tool's RETURN carries the envelope status `needs_review`** — two different fields, and both are correct. Doc 08's accept action (the human click) posts `draft_id` → the accept handler (Doc 04) reads the persisted bundle; **for a `code-change` draft it records approval and exposes the diff/branch bundle for download — it NEVER pushes** (PR-push is Expansion, behind `contents:write`, §5). Relocated host-side; **the dead `review_session` is deleted.**

> **P8 correction (2026-07-29).** P8 set out to fix one real defect: this doc used `needs_review` for the `staged_drafts` **row** status, which collides with CANONICAL §4's enum (`proposed | accepted | rejected | applied`) and with Doc 04 §3.16.1, which reads the row as `'proposed'`. It changed four sites. **Three were right and stand: lines 56, 309 and 368.** The fourth — the `propose_change` code sample's `return ok({...})` — was **wrong and has been reverted.** That return is an *envelope*, not a row, and `needs_review` is a valid `EnvelopeStatus` (CANONICAL §1.2). The shipped code agrees and was correct all along: `drafts.py` writes the row `'proposed'` (`INSERT … VALUES (…, 'proposed')`) and returns `status="needs_review"` to the caller. P8 briefly made this doc describe behaviour the code does not have; it no longer does. The lesson is the one P8 itself named — *"three spellings for one concept is how a fifth status gets invented"* — and the fix is to keep the two fields **distinguishable**, not to collapse them onto one word.
...
async def propose_change(args):
    ...
    return ok({"draft_id": str(draft_id), "status": "needs_review",   # ENVELOPE status — see P8b note
