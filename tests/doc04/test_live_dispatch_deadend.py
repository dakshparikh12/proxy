"""DEAD END — the live in-meeting dispatch tests, preserved and SKIPPED.

These nine tests passed. They proved a real regression fix: ``assemble_live_brain`` mounted
the MCP servers BEFORE it built the run loop, so an eager ``runtime.run_loop`` read made
``dispatch_workroom`` silently fail to mount on every live meeting.

``origin/main`` then deleted the spine they assert against — ``run_loop.py`` and
``live_brain.py`` are gone (``d00c158 delete-wave 1/6``), the ``services/harness`` member is
dissolved (``47712ca``), and the new ``services/in-meeting`` engine owns the boot path
(``6a6f4a8 CUTOVER``). There is no ``run_loop.queue`` to enqueue onto and no
``assemble_live_brain`` to inspect.

They are kept, unmodified below the skip, as the record of what the live path did and of the
ordering hazard any replacement inherits. The code under test lives in
``control_plane.live_dispatch_deadend``; that module's docstring holds the open founder
decision (this architecture vs. the in-meeting sandbox toolbelt) and records
``in_meeting.trigger.EngagementTrigger.on_worker_done`` as the structural replacement seam.

Skipped, not deleted, and not "fixed" to import the new paths — porting them would bake in an
architecture choice nobody has made.
"""
import pytest

pytest.skip(
    "live dispatch spine deleted by origin/main (run_loop + live_brain); see "
    "control_plane.live_dispatch_deadend for the open founder decision",
    allow_module_level=True,
)


# ─── original file preserved verbatim below ───
#
# Commented out, not deleted: it must PARSE (a SyntaxError here breaks doc04
# collection entirely) while importing nothing that no longer exists.
#
...
# from control_plane.dispatch import DISPATCH_SERVER_NAME, make_dispatch_workroom_tool
# from control_plane.dispatch_sinks import RunLoopUnavailable, live_sink
#
