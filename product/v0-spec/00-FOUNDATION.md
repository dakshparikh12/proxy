...
CREATE TABLE sessions (id uuid PK, user_id uuid REFERENCES users, tenant_id uuid REFERENCES tenants, created_at timestamptz);  -- DEVIATION (2026-07, founder-approved): relational typed-FK session store (HMAC-signed cookie over sessions.id → resolve_session → {user_id, tenant_id}). Supersedes connect-pg-simple (sid/data/expire) — no consumer needed it, typed FKs are safer. NON_SCOPED per AC-TEN-001.
```
