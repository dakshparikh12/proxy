...
- **DB-backed feature flags 4-tier** (VI.4) — ~~adopt a *simple* flags table + env floor for the 2–3 flags we actually need (`durable_meeting_sessions`, `proactive_enabled`). Skip the tenant→global→env→default resolver ceremony until we have per-tenant flag demand.~~ **SUPERSEDED (P9):** Doc 00 §7 cut the flags table for V0 — "env vars, no table" — and **both named flags are gone**: `durable_meeting_sessions` went with the Tier-2 mirror, `proactive_enabled` went with proactive. Doc 00's sealed criteria bundle tests for the *absence* of `proactive_enabled` in `libs/` and `services/`, so building this table would break a sealed oracle. V1 proactive uses one observability-only env var, not a table. This bullet is a catalog entry retained for provenance; do not implement it.
